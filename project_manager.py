...
class ProjectCreationWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Zusätzliche README erstellen
            readme_file = project_path / "README.md"
            with open(readme_file, 'w', encoding='utf-8') as f:
                f.write(f"""# {self.project_name}

## Projekt-Beschreibung
{self.description or 'Keine Beschreibung verfügbar'}
...
""")
            ...
            self.creation_finished.emit(True, f"Projekt '{self.project_name}' wurde erfolgreich erstellt!", str(project_path))
...
class ProjectManagerDialog(QDialog):
    """Robuster Dialog für Projekt-Management (Neu/Öffnen/Löschen)"""
    ...
    def init_ui(self):
        ...
        # Subtitle
        subtitle = QLabel("Wählen Sie ein bestehendes Projekt oder erstellen Sie ein neues")
        ...
        # Info-Label für Projekt-Details
        ...
        # Buttons für bestehende Projekte
        ...
        self.project_name_input.setPlaceholderText("z.B. Spritzguss_Qualitätskontrolle_2025")
    ...
    def validate_project_input(self):
        ...
        if not name:
            self.create_btn.setText("🚀 Projekt erstellen - Name erforderlich")
        elif len(name) < 3:
            self.create_btn.setText("🚀 Projekt erstellen - Name zu kurz")
        elif not path:
            self.create_btn.setText("🚀 Projekt erstellen - Pfad erforderlich")
        else:
            self.create_btn.setText("🚀 Projekt erstellen")
    
    def refresh_project_list(self):
        """Aktualisiert Liste der verfügbaren Projekte mit robuster Suche"""
        ...
        search_paths = [
            ...
            Path.home() / "Desktop"  # Desktop für einfachen Zugriff
        ]
        ...
        # Projekte zur Liste hinzufügen (sortiert nach letzter Änderung)
        for project in sorted(projects, key=lambda x: x['modified'], reverse=True):
            ...
            item_text = (f"📁 {project['name']}\n"
                        f"📍 {project['path']}\n"
                        f"📅 Erstellt: {created_date} | Geändert: {modified_date}\n"
                        f"🏷️ {project['classes']} Klassen | 🤖 {project['models']} Modelle")
        ...
        if not projects:
            no_projects_item = QListWidgetItem("❌ Keine Projekte gefunden\nErstellen Sie ein neues Projekt")
    ...
    def update_project_info(self):
        ...
        if not current_item:
            self.project_info_label.setText("Wählen Sie ein Projekt für Details")
            return
        
        project_data = current_item.data(Qt.ItemDataRole.UserRole)
        if not project_data:
            self.project_info_label.setText("Keine Projekt-Details verfügbar")
            return
...
Datei-Übersicht:
• Raw Images: {file_counts.get('01_raw_images', 0)} Dateien
• Labeled: {file_counts.get('02_labeled', 0)} Dateien  
• Augmented: {file_counts.get('03_augmented', 0)} Dateien
• Split: {file_counts.get('04_splitted', 0)} Dateien
• Models: {file_counts.get('05_models', 0)} Dateien
...
Modelle: {project_data['models']}"""
    ...
    def browse_project_directory(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Projekt-Verzeichnis wählen", current_path
        )
        if directory:
    ...
    def create_new_project(self):
        ...
        if project_path.exists():
            reply = QMessageBox.question(
                ...
                f"Möchten Sie es als neues AI Vision Tools Projekt initialisieren?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            if reply == QMessageBox.StandardButton.No:
                return
        
        # UI für Progress vorbereiten
        self.create_btn.setEnabled(False)
        self.progress_bar.setVisible(True)
        self.progress_label.setVisible(True)
        self.progress_bar.setValue(0)
        
        # Worker-Thread für Projekt-Erstellung starten
    ...
    def open_selected_project(self):
        """Öffnet ausgewähltes Projekt"""
        current_item = self.project_list.currentItem()
        if not current_item:
            QMessageBox.warning(self, "Auswahl-Fehler", "Bitte wählen Sie ein Projekt aus.")
            return
        
        project_data = current_item.data(Qt.ItemDataRole.UserRole)
        if not project_data:
            QMessageBox.warning(self, "Fehler", "Ungültige Projekt-Daten.")
            return
        ...
        if not Path(project_path).exists():
            QMessageBox.warning(
                ...
                f"Möglicherweise wurde es verschoben oder gelöscht."
            )
            self.refresh_project_list()
            return
    ...
    def delete_selected_project(self):
        ...
        reply = QMessageBox.question(
            ...
            f"⚠️ WARNUNG: Projekt unwiderruflich löschen? ⚠️\n\n"
            f"Projekt: {project_name}\n"
            f"Pfad: {project_path}\n\n"
            f"Dies löscht ALLE Dateien in diesem Verzeichnis!\n"
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Erfolg", f"Projekt '{project_name}' wurde gelöscht.")
                self.refresh_project_list()
            except PermissionError:
                QMessageBox.critical(
                    self, "Berechtigung verweigert", 
                    f"Keine Berechtigung zum Löschen des Projekts.\n"
                    f"Möglicherweise sind Dateien in Verwendung."
                )
            except Exception as e:
                QMessageBox.critical(self, "Fehler", f"Projekt konnte nicht gelöscht werden:\n{str(e)}")
    
    def get_selected_project(self) -> Optional[str]:
        """Gibt den Pfad des ausgewählten Projekts zurück"""
...
class WorkflowStatusWidget(QWidget):
    ...
    def show_error_state(self):
        """Zeigt Fehler-Status an falls Initialisierung fehlschlägt"""
        layout = QVBoxLayout(self)
        error_label = QLabel("⚠️ Fehler beim Laden des Workflow-Status")
    ...
    def init_ui(self):
        ...
        self.steps = {
            ...
            WorkflowStep.TRAINING: {
                ...
                'icon': '🚀',
                'description': 'YOLO-Training durchführen'
            },
        ...
        # Erstelle Widgets für jeden Schritt
        for i, (step, info) in enumerate(self.steps.items()):
            try:
                ...
                # Grid-Position berechnen (3 Spalten für bessere Darstellung)
                row = i // 3
                col = i % 3
        ...
        try:
            ...
            for step, widgets in self.step_widgets.items():
                try:
                    ...
                    # Tooltip mit Validierungs-Message (gekürzt für bessere Lesbarkeit)
                    tooltip_message = message if message else "Klicken zum Öffnen"
...
class ProjectManagerError(Exception):
    """Custom Exception für Projekt-Manager Fehler"""
    pass
...
def setup_project_logging(project_path: Path):
    ...
    try:
        ...
        project_logger.info(f"Projekt-Logging initialisiert für: {project_path.name}")
...
def safe_json_load(file_path: Path, default_value=None):
    ...
    try:
        ...
        logger.error(f"Ungültige JSON in {file_path}: {e}")
        return default_value
    except Exception as e:
...
def diagnose_project_issues(project_path: Path) -> Dict[str, any]:
    """Diagnostiziert häufige Projekt-Probleme"""
    diagnosis = {
    ...
    try:
        if not project_path.exists():
            ...
            diagnosis['recommendations'].append("Projekt-Verzeichnis erstellen oder korrekten Pfad wählen")
            return diagnosis
        ...
        if not diagnosis['readable']:
            diagnosis['issues'].append("Keine Leseberechtigung für Projekt-Verzeichnis")
            diagnosis['recommendations'].append("Berechtigung für Verzeichnis prüfen")
        
        if not diagnosis['writable']:
            diagnosis['issues'].append("Keine Schreibberechtigung für Projekt-Verzeichnis")
            diagnosis['recommendations'].append("Schreibberechtigung für Verzeichnis gewähren")
        ...
        if config_file.exists():
            ...
            if config_data:
                ...
                diagnosis['issues'].append("Projekt-Konfiguration ist beschädigt")
        ...
        diagnosis['recommendations'].append("Projekt-Pfad und -Berechtigung prüfen")
    
    return diagnosis

def create_diagnostic_report(project_path: Path) -> str:
    """Erstellt einen diagnostischen Bericht für ein Projekt"""
...
## Status-Übersicht
- Verzeichnis existiert: {'✅' if diagnosis['exists'] else '❌'}
- Leseberechtigung: {'✅' if diagnosis['readable'] else '❌'}
- Schreibberechtigung: {'✅' if diagnosis['writable'] else '❌'}
- Struktur gültig: {'✅' if diagnosis['structure_valid'] else '❌'}
- Konfiguration gültig: {'✅' if diagnosis['config_valid'] else '❌'}
...
"""
    ...
    if diagnosis['recommendations']:
        ...
        report += "✅ Keine Aktionen erforderlich\n"
...
class ProjectManager:
    ...
    def _create_directory_structure(self):
        ...
        # Spezielle Unterordner für Split mit Fehlerbehandlung
        try:
    ...
    def _load_or_create_config(self) -> ProjectConfig:
        ...
        if self.config_file.exists():
            try:
                ...
                logger.error(f"Ungültige JSON-Konfiguration: {e}")
    ...
    def _get_default_value(self, field: str):
        """Gibt Standard-Werte für fehlende Konfigurationsfelder zurück"""
        defaults = {
    ...
    def get_split_dir(self) -> Path:
        """Verzeichnis für Train/Val/Test Split"""
    ...
    def get_verification_dir(self) -> Path:
        """Verzeichnis für Verifikations-Resultate"""
    ...
    def add_class(self, class_id: int, class_name: str, color: str = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Klasse hinzugefügt: {class_id} = {class_name} ({color})")
            return True
    ...
    def get_class_colors(self) -> Dict[int, str]:
        """Gibt alle Klassenfarben zurück"""
        return self.config.class_colors.copy()
    
    def _update_yaml_file(self):
        """Aktualisiert YAML-Datei für YOLO-Training"""
        try:
            yaml_data = {
    ...
    def register_new_model(self, source_model_path: str, accuracy: float = None, 
        ...
        try:
            ...
            # Prüfe ob Quell-Modell existiert
    ...
    def delete_model(self, timestamp: str) -> bool:
        """Löscht Modell (Datei + Registrierung)"""
        try:
            model_info = next((m for m in self.config.models if m['timestamp'] == timestamp), None)
            if not model_info:
                logger.warning(f"Modell zum Löschen nicht gefunden: {timestamp}")
                return False
            ...
            if model_path.exists():
                model_path.unlink()
                logger.info(f"Modell-Datei gelöscht: {model_path}")
            ...
            self.save_config()
            logger.info(f"Modell erfolgreich gelöscht: {timestamp}")
            return True
    ...
    def _validate_labeled_data(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, f"Fehler beim Prüfen der gelabelten Daten: {str(e)}"
    
    def _validate_augmented_data(self) -> Tuple[bool, str]:
        """Validiert ob augmentierte Daten vorhanden sind"""
        try:
            # Falls Augmentation übersprungen wird, prüfe labeled data
            ...
            return False, f"Fehler beim Prüfen der augmentierten Daten: {str(e)}"
    ...
    def _validate_split_data(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, f"Fehler beim Prüfen der Split-Daten: {str(e)}"
    ...
    def get_augmentation_settings(self) -> Dict:
        """Gibt Augmentation-Einstellungen zurück"""
    ...
    def prepare_continual_training(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            # Prüfe ob neues Material vorhanden ist
            raw_dir = self.get_raw_images_dir()
            labeled_dir = self.get_labeled_dir() 
            
            # Neue Bilder seit letztem Training finden
            if not self.config.models:
                return False, "Kein Basis-Modell für Continual Learning gefunden.", {}
            ...
            # Training-Parameter für Continual Learning
            training_params = {
                'resume': str(self.get_current_model_path()),  # Wichtig für Ultralytics!
                'data': str(self.get_yaml_file()),
                'epochs': 50,  # Weniger Epochen für Fine-tuning
                'lr0': 0.001,  # Niedrigere Learning Rate
                'project': str(self.get_models_dir()),
                'name': f'continual_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
            }
            
            return True, f"{len(new_images)} neue Bilder für Continual Learning gefunden.", training_params
