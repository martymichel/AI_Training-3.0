...
from PyQt6.QtWidgets import (
    ...
    QProgressBar, QLabel, QPushButton, QHBoxLayout, QMessageBox
)
...
class TrainSettingsWindow(QMainWindow):
    ...
    def start_training(self):
        ...
        if not data_path:
            QMessageBox.critical(self, "Validierungsfehler", "Bitte wählen Sie eine YAML-Datei aus.")
            return
        ...
        start_training_thread(
            ...
            box, dropout, self.project, self.experiment
        )
    ...
    def update_progress(self, progress, message=""):
        ...
        try:
            if progress >= 100:
                ...
                self.disable_settings(False)
                self.results_check_timer.stop()
                QMessageBox.critical(self, "Fehler", f"Training fehlgeschlossen:\n{message}")
            else:
    ...
    def check_results_csv(self):
        ...
        df = check_and_load_results_csv(self.project, self.experiment, self.last_results_check)
        if df is not None:
            ...
            try:
                ...
                self.progress_bar.setFormat(f"%p% ({current_epoch}/{total_epochs} epochs)")
                self.progress_detail.setText(f"Training epoch {current_epoch}/{total_epochs}")
            except Exception as e:
